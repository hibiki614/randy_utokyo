...
# 総当たり探索
# =====================
def optimize_one_cum(L01, L12, L23, speed_kmh, tie_tol=1e-9):
    """
    総当たり探索（grid^3）で dopt(s/veh) 最小を探索する。
    同値最適（ties）があり得るので、最小値に一致するオフセットを全て返す。

    Returns
    -------
    best : dict
        代表の最適解（ties の先頭）。df へ書き戻す用。
    ties : list[dict]
        dopt が best と同値（|d - d*|<=tie_tol）の解を全て格納。
    """
    best = None
    best_val = float("inf")
    ties = []
    ...
    for x1 in grid:
        for x2 in grid:
            for x3 in grid:
                ...
                val = float(res["dopt(s/veh)"])

                # 新しい最小値を見つけたら ties をリセット
                if val < best_val - tie_tol:
                    best_val = val
                    best = res
                    ties = [res]
                # 同値最適（ties）
                elif abs(val - best_val) <= tie_tol:
                    ties.append(res)
                ...
                if checked % report_every == 0:
                    ...
                    # best が None のことは無いはずだが念のため
                    bx1 = best["x1opt"] if best is not None else float("nan")
                    bx2 = best["x2opt"] if best is not None else float("nan")
                    bx3 = best["x3opt"] if best is not None else float("nan")
                    print(
                        f"[{checked:,}/{total:,}={pct:6.2f}%] "
                        f"elapsed={elapsed:8.1f}s ETA={eta:8.1f}s "
                        f"best_dopt={best_val:.6f} "
                        f"best_x=({bx1:.1f},{bx2:.1f},{bx3:.1f}) "
                        f"ties={len(ties)}"
                    )
...
for idx in idxs:
    ...
    best, ties = optimize_one_cum(L01, L12, L23, spd)

    # 同値最適（ties）をシートに出すために溜める
    for j, t in enumerate(ties, start=1):
        row = {
            "路線番号": int(r["路線番号"]),
            "系統速度": float(spd),
            "link(0,1)": float(L01),
            "link(1,2)": float(L12),
            "link(2,3)": float(L23),
            "ties_count": int(len(ties)),
            "tie_rank": int(j),
        }
        for k in ['x0opt', 'x1opt', 'x2opt', 'x3opt', 'dopt(s/veh)', 'FTT(s/veh)', 'TTT(s/veh)', 'd0_stop(s/veh)', 'd1_stop(s/veh)', 'd2_stop(s/veh)', 'd3_stop(s/veh)', 'd_stop_total(s/veh)', 'Nveh_total_eval']:
            row[k] = float(t.get(k, np.nan))
        all_ties_rows.append(row)
...
# =====================
# 追加ユーティリティ：CSV/路線別シート/速度間共通オフセット集計
# =====================
from pathlib import Path

CSV_DIR = "all_opt_offsets_csv"
WRITE_ROUTE_TIES_SHEETS = True   # 路線別の ties を Excel にも載せる
INCLUDE_ALL_TIES_SHEET = False   # 巨大なので既定では False
COMMON_USE_RELATIVE = False      # False: (x0,x1,x2,x3) の完全一致, True: 差分パターンで比較
COMMON_DECIMALS = 10             # 丸め桁（浮動小数対策）
TOP_COMMON_PAIRS_PER_ROUTE = 20  # 路線ごとの見やすい上位表
...
def _offset_key_from_row(row, use_relative=COMMON_USE_RELATIVE, decimals=COMMON_DECIMALS):
    x0 = float(row.get("x0opt", 0.0))
    x1 = float(row["x1opt"])
    x2 = float(row["x2opt"])
    x3 = float(row["x3opt"])
    if use_relative:
        vals = (x1 - x0, x2 - x1, x3 - x2)
    else:
        vals = (x0, x1, x2, x3)
    return tuple(round(float(v), decimals) for v in vals)


def build_common_offset_tables(ties_df, use_relative=COMMON_USE_RELATIVE, decimals=COMMON_DECIMALS):
    """
    各路線について、速度ごとの最適オフセット集合の共通数を計算。
    出力:
      pair_df: long format (route, speed_a, speed_b, common_count, ...)
      matrix_dict: {route_id: DataFrame}  count matrix
      top_pairs_dict: {route_id: DataFrame} 見やすい上位表
    """
    if ties_df is None or len(ties_df) == 0:
        return pd.DataFrame(), {}, {}

    work = ties_df.copy()
    work["route_id"] = work["路線番号"].astype(int)
    work["speed_val"] = work["系統速度"].map(_norm_speed)
    work["offset_key"] = work.apply(
        lambda r: _offset_key_from_row(r, use_relative=use_relative, decimals=decimals),
        axis=1,
    )
    ...
    for route_id, g in work.groupby("route_id"):
        speed_to_set = {}
        for spd, gg in g.groupby("speed_val"):
            speed_to_set[spd] = set(gg["offset_key"].tolist())

        speeds = sorted(speed_to_set.keys())
        mat = pd.DataFrame(index=speeds, columns=speeds, dtype=float)

        for i, sa in enumerate(speeds):
            A = speed_to_set[sa]
            for j, sb in enumerate(speeds):
                B = speed_to_set[sb]
                inter = A & B
                union = A | B
                common_count = len(inter)
                union_count = len(union)
                min_size = min(len(A), len(B)) if len(A) and len(B) else 0
                overlap_ratio = (common_count / min_size) if min_size > 0 else np.nan
                jaccard = (common_count / union_count) if union_count > 0 else np.nan
                mat.loc[sa, sb] = common_count
                ...
                if j >= i:
                    pair_rows.append({
                        ...
                        "overlap_ratio_vs_smaller": float(overlap_ratio) if pd.notna(overlap_ratio) else np.nan,
                        "jaccard": float(jaccard) if pd.notna(jaccard) else np.nan,
                    })

        matrix_dict[int(route_id)] = mat

    pair_df = pd.DataFrame(pair_rows)
    if len(pair_df) > 0:
        top_df = pair_df[pair_df["speed_a"] != pair_df["speed_b"]].copy()
        top_df = top_df.sort_values(
            ["route_id", "common_count", "overlap_ratio_vs_smaller", "jaccard", "speed_a", "speed_b"],
            ascending=[True, False, False, False, True, True],
        )
        for route_id, g in top_df.groupby("route_id"):
            top_pairs_dict[int(route_id)] = g.head(TOP_COMMON_PAIRS_PER_ROUTE).reset_index(drop=True)
    else:
        top_pairs_dict = {}

    return pair_df, matrix_dict, top_pairs_dict


def safe_sheet_name(name, used=None, max_len=31):
    invalid = set('[]:*?/\\')
    clean = ''.join('_' if c in invalid else c for c in str(name))
    clean = clean[:max_len]
    if used is None:
        return clean
    base = clean
...
# =====================
# 1st write を差し替え：CSV + 路線別シート + 共通数表
# =====================
out_path = "results0.5ver.xlsx"
try:
    ties_df = pd.DataFrame(all_ties_rows)
except Exception:
    ties_df = pd.DataFrame()
...
if not ties_df.empty:
    ties_df.to_csv(csv_dir / "all_opt_offsets.csv", index=False, encoding="utf-8-sig")
    for route_id, g in ties_df.groupby("路線番号"):
        g.to_csv(csv_dir / f"route_{int(route_id)}_all_opt_offsets.csv", index=False, encoding="utf-8-sig")

# 共通オフセット集計（exact offset 完全一致ベース）
common_pairs_df, common_matrix_dict, common_top_dict = build_common_offset_tables(
    ties_df,
    use_relative=COMMON_USE_RELATIVE,
    decimals=COMMON_DECIMALS,
)
if len(common_pairs_df) > 0:
    common_pairs_df.to_csv(csv_dir / "common_offset_pairs_long.csv", index=False, encoding="utf-8-sig")
    for route_id, g in common_pairs_df.groupby("route_id"):
        g.to_csv(csv_dir / f"route_{int(route_id)}_common_offset_pairs_long.csv", index=False, encoding="utf-8-sig")
        if int(route_id) in common_matrix_dict:
            common_matrix_dict[int(route_id)].to_csv(
                csv_dir / f"route_{int(route_id)}_common_offset_count_matrix.csv",
                encoding="utf-8-sig"
            )

with pd.ExcelWriter(out_path, engine="openpyxl") as writer:
    used_sheet_names = set()
    df.to_excel(writer, sheet_name=safe_sheet_name("results", used_sheet_names), index=False)

    # ties は全体シートではなく路線別を基本に
    if INCLUDE_ALL_TIES_SHEET and not ties_df.empty:
        ties_df.to_excel(writer, sheet_name=safe_sheet_name("all_opt_offsets", used_sheet_names), index=False)

    if WRITE_ROUTE_TIES_SHEETS and not ties_df.empty:
        for route_id, g in ties_df.groupby("路線番号"):
            sheet = safe_sheet_name(f"ties_r{int(route_id)}", used_sheet_names)
            g.to_excel(writer, sheet_name=sheet, index=False)

    # 共通数の long と上位表
    if len(common_pairs_df) > 0:
        common_pairs_df.to_excel(writer, sheet_name=safe_sheet_name("common_pairs_long", used_sheet_names), index=False)
        for route_id, g in common_top_dict.items():
            sheet = safe_sheet_name(f"top_common_r{int(route_id)}", used_sheet_names)
            g.to_excel(writer, sheet_name=sheet, index=False)
        for route_id, mat in common_matrix_dict.items():
            sheet = safe_sheet_name(f"commonM_r{int(route_id)}", used_sheet_names)
            mat.to_excel(writer, sheet_name=sheet, index=True)
...
df = pd.read_excel(IN_PATH, sheet_name="results")
try:
    ties_df_in = pd.read_csv(Path(CSV_DIR) / "all_opt_offsets.csv")
except Exception:
    ties_df_in = None

common_pairs_in, common_matrix_in, common_top_in = build_common_offset_tables(
    ties_df_in if ties_df_in is not None else pd.DataFrame(),
    use_relative=COMMON_USE_RELATIVE,
    decimals=COMMON_DECIMALS,
)
...
for idx in idxs:
    ...
    totalF_stop = d1F + d2F + d3F
    totalB_stop = d2B + d1B + d0B
...
with pd.ExcelWriter(OUT_PATH, engine="openpyxl") as writer:
    ...
    if ties_df_in is not None and len(ties_df_in) > 0 and WRITE_ROUTE_TIES_SHEETS:
        for route_id, g in ties_df_in.groupby("路線番号"):
            sheet = safe_sheet_name(f"ties_r{int(route_id)}", used_sheet_names)
            g.to_excel(writer, sheet_name=sheet, index=False)

    if len(common_pairs_in) > 0:
        common_pairs_in.to_excel(writer, sheet_name=safe_sheet_name("common_pairs_long", used_sheet_names), index=False)
        for route_id, g in common_top_in.items():
            sheet = safe_sheet_name(f"top_common_r{int(route_id)}", used_sheet_names)
            g.to_excel(writer, sheet_name=sheet, index=False)
        for route_id, mat in common_matrix_in.items():
            sheet = safe_sheet_name(f"commonM_r{int(route_id)}", used_sheet_names)
            mat.to_excel(writer, sheet_name=sheet, index=True)
print("Wrote:", OUT_PATH)
