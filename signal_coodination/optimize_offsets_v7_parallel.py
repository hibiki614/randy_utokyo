...
# -*- coding: utf-8 -*-
"""
optimize_offsets_v7_parallel.py  (continuous-time fluid queue / event-driven) + multiprocessing

- 3リンク(0-1-2-3)のオフセット x1,x2,x3 を step (=0.01 cycle fraction) 刻みで総当たり
...
- 並列化: x1 グリッドをブロック分割し、各プロセスが部分探索→top10を返し、親でマージ

実行例(Windows推奨: コマンドラインで):
  python optimize_offsets_v7_parallel.py experiment.xlsx --out experiment_out.xlsx --jobs 12 --chunk-size 2

注意:
- multiprocessing の都合で、Spyder/Jupyter の %runfile より cmd 実行推奨
"""
...
def read_config(cfg):
    ...
    routes = _parse_list_or_all(
        kv.get("Routes_to_run (comma or 'all')", kv.get("Routes_to_run (comma)", None)),
        kind="int"
    )
...
def simulate_direction(path_nodes, taus, C, P, sat, xs, g, t_start_demand):
    """
    Event-driven continuous-time simulation of a FIFO fluid point queue in series with pure delays.

    path_nodes: signalized nodes in order (source first, last is last signal before sink)
    taus: link travel times between consecutive nodes, includes final link to sink as last element
    Demand: exogenous arrival at src at rate sat during [t_start_demand, t_start_demand+P*C)
            (red => accumulates at src queue)
    """
    demand_T = P * C
    ...
    a = {n: 0.0 for n in path_nodes}   # arrival rate into node queue
    s = {n: 0.0 for n in path_nodes}   # service rate (sat or 0)
    d = {n: 0.0 for n in path_nodes}   # departure rate
    delay = {n: 0.0 for n in path_nodes}  # veh*s integral
    ...
    # initial signal states and next switches
    next_switch = {}
    for n in path_nodes:
    ...
    while ev < max_events:
        ...
        # arrival-rate changes due to upstream departures
        ...
        if dt > 0:
            # integrate delay and update queues (net constant on (t,t_next))
            for n in path_nodes:
                net = a[n] - d[n]
        ...
        # demand start/end at src
        if abs(t - t_start_demand) <= 1e-9:
            a[src] = sat
        if abs(t - t_end_demand) <= 1e-9:
            a[src] = 0.0

        # scheduled arrival changes
        for n in path_nodes[1:]:
            changed, rate = sched[n].pop_all_at(t)
            if changed:
                a[n] = rate

        # recompute departures; propagate downstream if changed
...
def evaluate_offsets(tau01, tau12, tau23, C, P, sat, g, x1, x2, x3):
    # common offsets (shared across directions)
    xs = {0: 0.0, 1: x1, 2: x2, 3: x3}

    # forward 0->3 uses signals [0,1,2]
    delay_f, N_f, _T_f = simulate_direction(
        [0, 1, 2], [tau01, tau12, tau23],
        C, P, sat, xs, g, t_start_demand=xs[0] * C
    )

    # backward 3->0 uses signals [3,2,1]
    delay_b, N_b, _T_b = simulate_direction(
        [3, 2, 1], [tau23, tau12, tau01],
        C, P, sat, xs, g, t_start_demand=xs[3] * C
    )

    N_total = N_f + N_b  # should be 2*sat*P*C

    # node delays (veh*s) across both directions
    ...
    d_node = [D[i] / N_total for i in range(4)]  # s/veh
    d_total = sum(d_node)
    return d_node, d_total

# ---------------- parallel top-k search ----------------

def _merge_topk(heap, item, k):
    # item: (d_total, x1, x2, x3, d0, d1, d2, d3)
    packed = (-item[0], item[1], item[2], item[3], item[4], item[5], item[6], item[7])
    if len(heap) < k:
...
def _worker_x1_block(args):
    """
    args:
      (x1_list, grid, tau01, tau12, tau23, C, P, sat, g, k)
    returns:
      list[(d_total,x1,x2,x3,d0,d1,d2,d3)] sorted by d_total asc
    """
    x1_list, grid, tau01, tau12, tau23, C, P, sat, g, k = args
    heap = []
    for x1 in x1_list:
        for x2 in grid:
            for x3 in grid:
                d_node, d_total = evaluate_offsets(tau01, tau12, tau23, C, P, sat, g, x1, x2, x3)
                _merge_topk(heap, (d_total, x1, x2, x3, d_node[0], d_node[1], d_node[2], d_node[3]), k)

    res = [(-h[0], h[1], h[2], h[3], h[4], h[5], h[6], h[7]) for h in heap]
    res.sort(key=lambda x: x[0])
    return res

def topk_offsets_parallel(tau01, tau12, tau23, C, step, P, sat, g,
                          k=10, jobs=None, chunk_size=2, show_blocks=False):
    n = int(round(1 / step))
    ...
    x1_chunks = [grid[i:i + chunk_size] for i in range(0, len(grid), chunk_size)]
    tasks = [(x1_list, grid, tau01, tau12, tau23, C, P, sat, g, k) for x1_list in x1_chunks]
    ...
    with ctx.Pool(processes=jobs) as pool:
        for top_part in pool.imap_unordered(_worker_x1_block, tasks, chunksize=1):
            done += 1
            for row in top_part:
                _merge_topk(heap_all, row, k)
            if show_blocks:
                print(f"    blocks done: {done}/{len(tasks)}", flush=True)

    res = [(-h[0], h[1], h[2], h[3], h[4], h[5], h[6], h[7]) for h in heap_all]
    res.sort(key=lambda x: x[0])
    return res
...
def main(xlsx_path: str, out_path: str | None = None, sheet_name: str | None = None,
         jobs: int | None = None, chunk_size: int = 2, show_blocks: bool = False):
    ...
    print(f"Parallel: jobs={jobs}, chunk_size={chunk_size}", flush=True)
    ...
    for idx, (route, speed, r) in enumerate(candidates, start=1):
        ...
        top10 = topk_offsets_parallel(
            tau01, tau12, tau23, C, step, P, sat, g,
            k=10, jobs=jobs, chunk_size=chunk_size, show_blocks=show_blocks
        )
...
if __name__ == "__main__":
    ...
    ap.add_argument("--jobs", type=int, default=None, help="並列プロセス数（省略時はCPUコア数）")
    ap.add_argument("--chunk-size", type=int, default=2, help="x1を何個まとめて1タスクにするか（2〜5推奨）")
    ap.add_argument("--show-blocks", action="store_true", help="ブロック進捗（x1ブロック完了数）を表示")
    args = ap.parse_args()

    main(args.xlsx, args.out, args.sheet, jobs=args.jobs, chunk_size=args.chunk_size, show_blocks=args.show_blocks)
