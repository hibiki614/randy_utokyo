...
# 代表解（results）と、同値最適（all_opt_offsets）を別シートで保存
try:
...
with pd.ExcelWriter(out_path, engine="openpyxl") as writer:
    ...
    if not ties_df.empty:
        ties_df.to_excel(writer, sheet_name="all_opt_offsets", index=False)
...
# 同値最適シート（存在すれば）も引き継ぐ
try:
    ties_df_in = pd.read_excel(IN_PATH, sheet_name="all_opt_offsets")
except Exception:
    ties_df_in = None
...
with pd.ExcelWriter(OUT_PATH, engine="openpyxl") as writer:
    df.to_excel(writer, sheet_name="results", index=False)
    if ties_df_in is not None and len(ties_df_in) > 0:
        ties_df_in.to_excel(writer, sheet_name="all_opt_offsets", index=False)
print("Wrote:", OUT_PATH)